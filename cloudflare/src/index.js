...
async function importMobileSentrixParts(request, env) {
  ...
  return json({ok:true,supplier:'mobilesentrix-eu',imported:saved,eBayWrites:false},200,env.DASHBOARD_ORIGIN);
    ...
    const reason=/too many|limit|quota/i.test(message)?'Service request or database limit reached':/no such table/i.test(message)?'Required database table missing':/bind|NaN|type/i.test(message)?'Invalid supplier field for database storage':'Import service error';
    return json({ok:false,error:reason,stage,eBayWrites:false},502,env.DASHBOARD_ORIGIN);
  }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/ebay-deletion-inbox' && request.method === 'POST') {
      ...
      }catch(error) {const message=String(error?.message||error);const reason=/limit|quota|too many/i.test(message)?'DATABASE_LIMIT':/no such/i.test(message)?'DATABASE_SCHEMA':'DATABASE_ERROR';return json({ok:false,error:reason},503);}
    }
