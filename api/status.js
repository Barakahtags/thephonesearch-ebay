...
module.exports=async function(req,res){
  ...
  const out={ok:true,time:new Date().toISOString(),config:{mpsCredentials:!!(process.env.MPS_USERNAME&&process.env.MPS_PASSWORD),ebayStaticToken:!!process.env.EBAY_USER_TOKEN,ebayRefreshReady:!!(process.env.EBAY_CLIENT_SECRET&&process.env.EBAY_REFRESH_TOKEN),marketplace:process.env.EBAY_MARKETPLACE_ID||'EBAY_DE',currency:process.env.EBAY_CURRENCY||'EUR',publish:on('EBAY_PUBLISH'),syncMode:process.env.SYNC_MODE||'preview',automation:{supplierPurchase:on('MPS_AUTO_PURCHASE'),ebayTracking:on('EBAY_AUTO_TRACKING'),listingPublish:on('EBAY_PUBLISH')}}};
