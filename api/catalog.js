...
module.exports=async function(req,res){
  ...
  try{
    ...
    const items=(data.Parts||[]).map(p=>({id:p.Id,sku:p.PartNumber,title:p.Description,manufacturer:p.Manufacturer,stock:p.AvailableStockQuantity,costExVat:p.UnitPrice,eBayPrice:ebay.sellingPrice(p.UnitPrice),ean:p.EanNumber,orderable:p.CanBeOrdered,statusText:p.StatusText,averageDeliveryInDays:p.AverageDeliveryInDays,images:(p.Images||[]).map(x=>x.ImageUrl).filter(Boolean)}));
    res.status(200).json({ok:true,q,articleType,page,pageSize,total:data.TotalNumberOfParts,hasMore:data.HasMoreRecords,items});
