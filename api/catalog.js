...
module.exports=async function(req,res){
  ...
  try{
    ...
    const excludedOnPage=source.filter(catalogueExclusionReason).length;
    const items=source.filter(p=>!catalogueExclusionReason(p)).map(p=>{const images=imageUrls(p);return{id:p.Id,sku:p.PartNumber,title:p.Description,manufacturer:p.Manufacturer,stock:p.AvailableStockQuantity,costExVat:p.UnitPrice,eBayPrice:ebay.sellingPrice(p.UnitPrice),ean:p.EanNumber,orderable:p.CanBeOrdered,statusText:p.StatusText,averageDeliveryInDays:p.AverageDeliveryInDays,images,imageRecovery:images.length?{status:'SUPPLIER_IMAGE',source:'MOBILEPARTS_API',rightsBasis:'SUPPLIER_API'}:{status:'RECOVERY_REQUIRED'},secondaryArticleNumbers:p.SecondaryArticleNumbers||[],replacementArticleNumbers:p.ReplacementArticleNumbers||[]}});
    res.status(200).json({ok:true,q,articleType,articleTypeName:articleType===1?'Ersatzteile':'Werkzeuge',page,pageSize,total:data.TotalNumberOfParts,hasMore:data.HasMoreRecords,excludedOnPage,qualityRules:['PARTS_AND_TOOLS_ONLY',...QUALITY_RULES],items});
