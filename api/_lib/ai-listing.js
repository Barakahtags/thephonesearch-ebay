...
function chooseTitle(facts){const {brand,model,partType,quality,colour}=facts,alias=ENGLISH_ALIAS[partType]||'Spare Part';const authenticity=quality.filter(x=>x==='Original'||x==='OEM'),features=quality.filter(x=>x!=='Original'&&x!=='OEM');const prefix=[...authenticity,brand,model].filter(Boolean),item=[partType,alias].filter(Boolean);const candidates=uniq([
...
function buildDescription(f,title){const known=[],alias=ENGLISH_ALIAS[f.partType]||'Spare Part';known.push(`<li><strong>Artikel:</strong> ${esc(f.partType)} (${esc(alias)})</li>`);if(f.brand)known.push(`<li><strong>Marke:</strong> ${esc(f.brand)}</li>`);if(f.model)known.push(`<li><strong>Kompatibilität:</strong> ${esc(f.model)}</li>`);if(f.quality.length)known.push(`<li><strong>Ausführung:</strong> ${esc(f.quality.join(', '))}</li>`);if(f.colour)known.push(`<li><strong>Farbe:</strong> ${esc(f.colour)}</li>`);if(f.partNumber)known.push(`<li><strong>Herstellernummer / SKU:</strong> ${esc(f.partNumber)}</li>`);if(f.ean)known.push(`<li><strong>EAN:</strong> ${esc(f.ean)}</li>`);known.push('<li><strong>Zustand:</strong> Neu</li>');return `<div><h2>${esc(title)}</h2><p>Passendes ${esc(f.partType)} für die fachgerechte Reparatur von Smartphones und Tablets. Der englische Suchbegriff „${esc(alias)}“ dient ausschließlich als zusätzliche Artikelbezeichnung. Alle Produktangaben stammen aus den verfügbaren Lieferantendaten.</p><ul>${known.join('')}</ul><p><strong>Lieferumfang:</strong> 1x ${esc(f.partType)} wie beschrieben.</p><p><strong>Wichtig:</strong> Bitte vergleichen Sie vor dem Kauf Marke, Modell, Teilenummer, Ausführung und Farbe mit Ihrem vorhandenen Bauteil.</p><hr><p><strong>ThePhoneSearch</strong> – Ersatzteile und Werkzeuge für Smartphone- &amp; Tablet-Reparaturen.</p></div>`;}
