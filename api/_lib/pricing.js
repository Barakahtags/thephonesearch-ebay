const PRICING_VERSION='fixed-after-tax-profit-v6';
...
function breakdown(itemPrice,supplierCost,o={}){
  ...
  return {pricingVersion:PRICING_VERSION,itemPrice:round(p),customerShipping:round(customerShipping),totalRevenue:round(totalRevenue),salesVat:round(salesVat),supplierCost:round(cost),supplierShipping:round(supplierShipping),embeddedShippingCost:ship.embeddedShippingCost,shippingMode:ship.mode,combinedShipping:false,ebayProductFee:round(productFee),ebayProductFeeVat:round(productFeeVat),ebayFixedFee:round(c.fixedFee),ebayFixedFeeVat:round(fixedFeeVat),ebayShippingFee:round(shippingFee),ebayShippingFeeVat:round(shippingFeeVat),totalEbayCharges:round(totalEbayCharges),totalCostsBeforeProfitTax:round(totalCostsBeforeProfitTax),preTaxProfit:round(preTaxProfit),profitTaxReserve:round(profitTaxReserve),totalCosts:round(totalCosts),netProfit:round(netProfit),afterTaxProfit:round(netProfit),netMargin:Number((netMargin*100).toFixed(2)),targetProfit:round(targetProfit),profitPass,marginPass:profitPass,assumptions:{salesVatRate:c.salesVatRate,ebayProductFeeRate:c.ebayProductFeeRate,ebayShippingFeeRate:c.ebayShippingFeeRate,feeVatRate:c.feeVatRate,profitTaxReserveRate:c.profitTaxReserveRate,fixedFee:c.fixedFee,shippingAllocation:'8.40 reserved for every separate MPS customer shipment',customerShippingRule:'4.99 charged to the customer in Germany only',shippingDestination:'Germany only until verified international rates are configured',profitSchedule:'Strong fixed after-tax profit tiers'}};
}
...
function recommendedPrice(supplierCost,_requestedPrice=null,o={}){return minimumItemPrice(supplierCost,o);}
