...
async function imageDimensions(bytes){
  ...
  // MobileParts now returns WebP/AVIF for some originals. The old header-only
  // reader marked those successful images as "unknown" and blocked listings.
  // Sharp is already used by the image preview endpoint and reads these formats
  // without altering the original URL submitted to eBay.
  try{
    const metadata=await require('sharp')(bytes,{failOn:'none'}).metadata();
    if(Number(metadata?.width)>0&&Number(metadata?.height)>0)return{width:Number(metadata.width),height:Number(metadata.height)};
  }catch{}
  return null;
}
...
async function validEbayImages(part){
  ...
  for(const url of candidates){
    try{
      ...
      clearTimeout(timer);
      if(!response.ok){checked.push({host:new URL(url).hostname,status:response.status,size:null});continue;}
      const size=await imageDimensions(await response.arrayBuffer());
      checked.push({host:new URL(url).hostname,status:response.status,size:size?{width:size.width,height:size.height}:null});
      if(size&&Math.max(size.width,size.height)>=500)valid.push(url);
    }catch(error){checked.push({host:(()=>{try{return new URL(url).hostname}catch{return 'invalid-url'}})(),status:'error',size:null});}
  }
  if(!valid.length){
    console.warn(JSON.stringify({event:'image_preflight_rejected',sku:String(part?.PartNumber||part?.Id||''),candidates:checked}));
    const error=new Error('The latest MobileParts image links were saved, but none of the returned original image files is at least 500px. This item was not sent to eBay.');
...
async function upsertPart(p){
  ...
  const marketplace=process.env.EBAY_MARKETPLACE_ID||'EBAY_DE',
    ...
    // Use the agreed MobilePartsDE HTML generated by the local optimiser.
