...
function originalSupplierImage(url){const raw=String(url||'').trim();return /images\\.2service\\.nl\\/v7\\/Images\\/Part\\//i.test(raw)?raw.split('?')[0]:raw}\nasync function validEbayImages(part){const candidates=[...new Set(imageUrls(part).map(originalSupplierImage))].slice(0,12),valid=[];for(const url of candidates){try{const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),8000),response=await fetch(url,{signal:controller.signal,headers:{Accept:'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8'}});clearTimeout(timer);if(!response.ok)continue;const size=imageSize(await response.arrayBuffer());if(size&&Math.max(size.width,size.height)>=500)valid.push(url)}catch{}}if(!valid.length){const error=new Error('No original supplier image meets eBay’s 500px longest-edge requirement. This item was not sent to eBay.');error.status=422;throw error}return valid}
