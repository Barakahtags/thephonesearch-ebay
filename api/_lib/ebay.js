...
async function validEbayImages(part){const candidates=imageUrls(part).slice(0,12),valid=[];for(const url of candidates){try{const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),8000),response=await fetch(url,{signal:controller.signal,headers:{Accept:'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8'}});clearTimeout(timer);if(!response.ok)continue;const size=imageSize(await response.arrayBuffer());if(size&&Math.max(size.width,size.height)>=500)valid.push(url)}catch{}}if(!valid.length){const error=new Error('No original supplier image meets eBay’s 500px longest-edge requirement. This item was not sent to eBay.');error.status=422;throw error}return valid}
...
module.exports={api,token,ebaySku,simpleListingDescription,validEbayImages,policies,firstInventoryLocation,defaultCategoryTreeId,suggestedCategory,categoryAspects,sellingPrice,ebayAspects,orderFulfillments,createShippingFulfillment,setInventoryQuantityIfExists,upsertPart:safeUpsertPart,pricing};
