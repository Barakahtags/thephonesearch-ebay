...
async function competitorPrice(p,title,minimumPrice){
  ...
  const params=new URLSearchParams({q,limit:String(Math.min(50,Math.max(10,Number(process.env.EBAY_COMPETITOR_LIMIT||30))))});
  ...
  const rows=(data?.itemSummaries||[]).map(x=>{
    const price=num(x?.price?.value),ship=num(x?.shippingOptions?.[0]?.shippingCost?.value)||0;
    return {itemId:x.itemId,title:x.title,price,shipping:ship,total:price==null?null:price+ship,currency:x?.price?.currency||currency,condition:x.condition,webUrl:x.itemWebUrl};
  }).filter(x=>x.total!=null&&x.currency===currency);
  const totals=rows.map(x=>x.total);
  const med=median(totals),q1=percentile(totals,.25),q3=percentile(totals,.75);
  const iqr=q1!=null&&q3!=null?q3-q1:null;
  const filtered=iqr==null?rows:rows.filter(x=>x.total>=q1-1.5*iqr&&x.total<=q3+1.5*iqr);
  const marketMedian=median(filtered.map(x=>x.total));
  const marketLow=percentile(filtered.map(x=>x.total),.25);
  const target=marketMedian==null?Number(minimumPrice):Math.max(Number(minimumPrice),marketMedian);
  return {query:q,resultCount:rows.length,usableCount:filtered.length,marketLow:marketLow==null?null:round2(marketLow),marketMedian:marketMedian==null?null:round2(marketMedian),recommendedPrice:round2(target),minimumPrice:round2(minimumPrice),strategy:marketMedian==null?'30_PERCENT_FLOOR_ONLY':target>Number(minimumPrice)?'MARKET_MEDIAN_ABOVE_FLOOR':'30_PERCENT_FLOOR',sample:filtered.slice(0,8)};
}
