...
async function competitorPrice(p,title){
  ...
  if(rows.length<2&&exactIdentifierMatches<1)return{...base,recommendedItemPrice:null,recommendedBuyerTotal:null,expectedProfit:null,status:'INSUFFICIENT_MARKET_DATA',confidence:'LOW',reason:'Fewer than two reliable eBay matches were found. The protected fixed-profit price is used while the market check is retried.'};
  if(undercutItemPrice+0.01<target.itemPrice)return{...base,recommendedItemPrice:target.itemPrice,recommendedBuyerTotal:target.totalRevenue,expectedProfit:target.netProfit,status:'PROFIT_FLOOR_PRICE',confidence:rows.length>=4?'MEDIUM':'LOW',reason:'The competitor price is below the fixed-profit floor, so the protected fixed-profit price is used instead.'};
