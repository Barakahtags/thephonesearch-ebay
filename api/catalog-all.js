...
module.exports=async function(req,res){
  ...
  try{
    ...
    let page=1,items=[],total=0,hasMore=true;
    while(hasMore && page<=maxPages){
      ...
      items.push(...(data.Parts||[]).map(mapPart));
      hasMore=!!data.HasMoreRecords;
      page++;
    }
    res.status(200).json({ok:true,q,articleType,pageSize,total,retrieved:items.length,complete:!hasMore,pagesFetched:page-1,items});
