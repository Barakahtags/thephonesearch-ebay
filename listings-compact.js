...
listingCard=function(p,i){
  const r=stateFor(p),len=r.title.length,checked=SELECTED.has(p.sku)?' checked':'',profit=listingProfit(p);
  return `<article id="card-${i}" class="compactListing">
    <div class="compactRow">
      ...
      <div class="compactPrice"><span class="label">AI eBay price</span><b>${money(p.calculatedPrice)}</b><small class="muted">Cost ${money(p.costExVat)} · <strong class="${profit!=null&&profit>=0?'good':'bad'}">Profit ${profit==null?'calculating…':money(profit)}</strong></small><small class="muted">${esc(p.competitorPricing?.strategy||(p.pricing?'Automatic price saved':'Automatic market analysis pending'))}</small></div>
