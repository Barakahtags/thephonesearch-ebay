...
listingCard=function(p,i){
  ...
  return `<article id="card-${i}" class="compactListing">
    <div class="compactRow">
      ...
      <div class="compactPrice"><span class="label">AI eBay price</span><b>${money(p.calculatedPrice)}</b><small class="muted">Cost ${money(p.costExVat)}${p.pricing?` · Profit ${money(p.pricing.netProfit)}`:''}</small><small class="muted">${esc(p.competitorPricing?.strategy||'Run price optimizer')}</small></div>
