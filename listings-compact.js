...
function pricingMessage(p,pricingReady){
  if(pricingReady&&p.pricing?.customProfitTarget)return `Custom after-tax profit target ${money(p.pricing.targetProfit)}`;
  if(pricingReady)return 'Final fixed after-tax profit price';
  return 'Calculating fixed after-tax profit price';
}
...
function costChart(p,pricingReady){
  ...
  if(!pricingReady)return `<div class="compactCostChart"><span>Fixed-profit price</span><b>${amount(f.minimumItemPrice)}</b><div class="compactProfitEditor"><label>After-tax profit target (€)<input id="profit-target-${sku}" type="number" min="0" max="10000" step="0.01" value="${target.toFixed(2)}" onkeydown="if(event.key==='Enter')saveItemProfitTarget('${sku}')"></label><button id="profit-save-${sku}" class="btn approve" onclick="saveItemProfitTarget('${sku}')">Recalculate &amp; save</button></div></div>`;
...
listingCard=function(p,i){
  ...
  return `<article id="card-${i}" class="compactListing">
    <div class="compactRow">
      ...
      <div class="compactPrice"><span class="label">Final eBay item price</span><b class="${pricingReady?'good':'warn'}">${pricingReady?money(p.calculatedPrice):'Calculating…'}</b><small class="muted">Customer shipping ${money(p.pricing?.customerShipping??4.99)}${pricingReady?` · <strong class="${profit>=0?'good':'bad'}">Profit ${money(profit)}</strong>`:''}</small><small class="muted">${esc(pricingMessage(p,pricingReady))}</small>${costChart(p,pricingReady)}</div>
